...
//            console.log('token:', type, lookup[type], '`'+normalizedInput.substring(lastStart, pos).replace(/\n/g,'\u23CE')+'`');
        } while (type === true);
...
;// #################################
 // ## Start of file: par-alt.js
 // #################################

exports.ParAlt = function(input){
    this.tok = new Tok2(input);
};

exports.ParAlt.prototype = {
    run: function(){
        // prepare
        this.tok.nextExpr();
    ...
    parseContinue: function(){
        ...
        this.tok.next();
        if (this.tok.newlineBefore()) this.addAsi();
        else {
    ...
    parseSemi: function(){
        ...
        throw 'Unable to parse semi, unable to apply ASI: '+this.tok.debug()+' #### '+
            this.tok.substr(this.tok.start()-2000, this.tok.start())+
            '###'+
            this.tok.substr(this.tok.start(), this.tok.start()+2000);
    },
    parseAsi: function(){
        if (this.tok.is(EOF) || this.tok.is('}') || this.tok.newlineBefore()) {
            return this.addAsi();
        }
        return false;

...
var good = [
  ...
  // for-in LHS tests
  ["for (a in b);", 10, "base for-in expr case"],
  ["for (a.x in b);", 12, "for-in property"],
  ["for (a.x.y in b);", 14, "for-in double property"],
  ["for (a[x] in b);", 13, "for-in dyn property"],
  ["for (a[x in b] in c);", 17, "for-in dyn property containing in"],
  ["for (a().x in b);", 14, "for-in call property"],
  ["for (a(p in q).x in b);", 19, "for-in call containing in"],
  ["for (a[b](c).d in e);", 18, "for-in dyn-call-prop"],
  ["for (a[{k: x in y}].b in c);", 24, "for-in dyn containing objlit containing in"],
  ["for (a({k: x in y}).b in c);", 24, "for-in cal containing objlit containing in"],
  ["for (a[[x in y]].b in c);", 21, "for-in dyn containing arrlit containing in"],
  ["for (a([x in y]).b in c);", 21, "for-in call containing arrlit containing in"],
  ["for (a(x,y).b in c);", 17, "for-in call containing comma"],
  ["for (a[x,y].b in c);", 17, "for-in dyn containing comma"],
  ["for (a([x,y]).b in c);", 19, "for-in call containing arrlit containing comma"],
  ["for (a[[x,y]].b in c);", 19, "for-in dyn containing arrlit containing comma"],
  ["for (var a in b);", 12, "base for-in var case"],
  ["for (var a=1 in b);", 14, "base for-in var with initializer"],
  ["for (var a=(x in y) in b);", 20, "for-in initializer containing in"],
  ["for (var a=function(){} in b);", 18, "for-in initializer function"],
  ["for (var a=function(){ x in y; } in b);", 26, "for-in initializer function containing in"],
  ["for (var a=[x in y] in c);", 20, "for-in initializer arrlit containing in"],
  ["for (var a={k:x in y} in b);", 22, "for-in initializer objlit containing in"],
  ["for (var a=f(x in y) in b);", 21, "for-in initializer call containing in"],
  ["for (var a=f[x in y] in b);", 21, "for-in initializer dyn containing in"],
  ["for (var a=f([x in y]) in b);", 23, "for-in initializer call containing arr containing in"],
  ["for (var a=f[[x in y]] in b);", 23, "for-in initializer dyn containing arr containing in"],
  ["for (var a=f({k:x in y}) in b);", 25, "for-in initializer call containing objlit containing in"],
  ["for (var a=f[{k:x in y}] in b);", 25, "for-in initializer dyn containing objlit containing in"],
  ["for (var a=b?c:d in e);", 18, "for-in initializer with ternary"],
  ["for (var a=b?c in d:e in f);", 22, "for-in initializer with ternary containing in"],
  ["for (a,b;;);", 10, "for-loop with comma start"],
  // for-in no-var lhs wrapped in parens
  ["for ((a) in b);", 12, "for-in paren 1"],
  ["for ((a[x]) in b);", 15, "for-in paren 2"],
  ["for ((a[x in b]) in c);", 19, "for-in paren 3"],
  ["for ((a().x) in b);", 16, "for-in paren 4"],
  ["for ((a(p in q).x) in b);", 21, "for-in paren 5"],
  ["for ((a[b](c).d) in e);", 20, "for-in paren 6"],
  ["for (((a)) in b);", 14, "for-in paren 7"],
  ["for (((a[x])) in b);", 17, "for-in paren 8"],
  ["for (((a[x in b])) in c);", 21, "for-in paren 9"],
  ["for (((a().x)) in b);", 18, "for-in paren 10"],
  ["for (((a(p in q).x)) in b);", 23, "for-in paren 11"],
  ["for (((a[b](c).d)) in e);", 22, "for-in paren 12"],
  ["for ((x in b); c; u) {};", 21, "`in` wrapped in parens as first part of for-loop"],
  ["for ((a in b).foo in c);", 18, "for-in with parens lhs `in` that ends with property"],
  ["for ((a,b).foo in c);", 16, "for-in with parens lhs comma that ends with property"],
  ["for (('x' in {}).c in {key:1});", 23, "for-in superweird case - in"],
  ["for ((1?2:'3'in{}).foo in {key:1});", 25, "for-in superweird case - ternary"],
  ["for ((x = [x in y]).foo in z);", 24, "odd in construct even though array clearly incorrect"],
  ["for ((x = {x:x in y}).bar in z);", 26, "odd in construct even though array clearly incorrect"],
  ["for (var a=b in c ? d : e in f);", 26, "`in` after the lhs of a ternary is ok"],
...
var bad = [
  ...
  ["foo=;", "incomplete assignment"],
  ["foo+;", "incomplete addition"],
  ["foo/;", "incomplete division"],
  ["foo*;", "incomplete multiplication"],
  ["for( in y);", "empty for-in lhs"],
  ["for(* in y);", "incomplete multiplication in for-in"],
  ["for(x* in y);", "incomplete multiplication in for-in"],
  ["for(a?b:x* in y);", "incomplete multiplication in for-in"],
  ["for(a?b:* in y);", "incomplete multiplication in for-in"],
  ["for(var x=a* in y);", "empty for-in lhs"],
  ["for(var x=u?i:a* in y);", "empty for-in lhs"],
  ["for(*;;);", "incomplete multiplication in for-loop"],
  ["for(x*;;);", "incomplete multiplication in for-loop"],
  ["for(a?b:x*;;);", "incomplete multiplication in for-loop"],
  ["for(a?b:*;;);", "incomplete multiplication in for-loop"],

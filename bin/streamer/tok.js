(function(exports){
  ...
  var proto = {
    ...
    getMoreInput: function(mustHaveMore){
      ...
      do {
        if (mustHaveMore) {
          if (!this.throwIfNotStreaming()) {
            this.throwSyntaxError('Unexpected EOF');
          }
    ...
    next: function(expressionStart){
      ...
      do {
        ...
        if (saveTokens) {
          var token = {type:type, value:this.getLastValue(), start:this.lastOffset, stop:this.pos, white:this.tokenCountAll, before:'', after: ''};
          tokens.push(token);
        }
        if (onToken) {
    ...
    nextAnyToken: function(expressionStart){
      ...
      if (fullStart >= this.len && !this.getMoreInput(false)) {
        this.firstTokenChar = 0;
        return EOF;
      }
...
//        case ORD_ZWS: // not accepted AFAICS
    ...
    parseFwdSlash: function(expressionStart){
      if (this.pos+1 >= this.len) this.getMoreInput(true);
    ...
    parseCR: function(){
      ...
      var pos = this.pos;
      if (pos+1 >= this.len) this.getMoreInput(false); // optional
    ...
    parseVerifiedNewline: function(pos, extraForCrlf){
      ...
      while (++pos < this.len) {
        ...
        if (saveTokens) {
          ...
          tokens.push({type:WHITE, value:v, start:s, stop:pos, white:count, before:'', after: ''});
        }
        if (onToken) {
          var s = pos-(1+extraForCrlf);
          var v = input.slice(s, pos);
    ...
    parseSameOrCompound: function(c){
      ...
      var pos = this.pos+1;
      if (pos >= this.len) this.getMoreInput(true);
...
//      this.pos += 1 + !(d-c && d-ORD_IS_3D);
    ...
    parseEqualSigns: function(){
      ...
      var input = this.input;
      if (offset+1 >= this.len) this.getMoreInput(true);
      if (input.charCodeAt(offset+1) === ORD_IS_3D) {
        if (offset+2 >= this.len) this.getMoreInput(true);
    ...
    parseLtgtPunctuator: function(c){
      ...
      if (d === ORD_IS_3D) len = 2;
        ...
        if (e === ORD_IS_3D) len = 3;
          ...
          len = 3;
          if (offset+3 >= this.len) this.getMoreInput(true);
    ...
    parseSingleComment: function(){
      ...
      do {
        if (++pos >= this.len && !this.getMoreInput(false)) break;
        var c = input.charCodeAt(pos);
...
*/
    ...
    parseStringEscape: function(pos){
      ...
      // line continuation; skip windows newlines as if they're one char
        ...
        if (pos+1 >= this.len) this.getMoreInput(true);
        if (input.charCodeAt(pos+1) === ORD_LF_0A) ++pos;
      // hex escapes
      } else if (c === ORD_L_X_78) {
        if (pos+1 >= this.len) this.getMoreInput(true);
        if (pos+2 >= this.len) this.getMoreInput(true);
    ...
    parseUnicodeEscapeBody: function(pos){
      ...
      if (pos+1 >= this.len) this.getMoreInput(true);
      if (pos+2 >= this.len) this.getMoreInput(true);
      if (pos+3 >= this.len) this.getMoreInput(true);
    ...
    parseLeadingZero: function(){
      ...
      if (this.pos+1 >= this.len && !this.getMoreInput(false)) {
        ++this.pos;
        return NUMBER;
      } // 0 at eof can be valid
    ...
    parseAfterDot: function(pos){
      var input = this.input;
      if (pos < this.len || this.getMoreInput(false)) {
        do { var c = input.charCodeAt(pos);
        } while (c >= ORD_L_0_30 && c <= ORD_L_9_39 && (++pos < this.len || this.getMoreInput(false)));
      }
    ...
    parseExponent: function(c, pos, input){
      if (c === ORD_L_E_65 || c === ORD_L_E_UC_45) {
        ...
        if (c === ORD_DASH_2D || c === ORD_PLUS_2B) {
          if (++pos >= this.len) this.getMoreInput(true); // must have at least one char after +-
          c = input.charCodeAt(pos);
        }

        // first digit is mandatory
        if (c >= ORD_L_0_30 && c <= ORD_L_9_39) {
          if (++pos >= this.len && !this.getMoreInput(false)) return pos;
          c = input.charCodeAt(pos);
        }
    ...
    regexBody: function(){
      ...
      while (this.pos < len) {
        if (this.pos >= this.len) this.getMoreInput(true);
    ...
    regexFlagUniEscape: function(input, pos){
      ...
      if (pos+4 >= this.len) this.getMoreInput(true);

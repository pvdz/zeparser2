...
//     * @property {boolean} [options.scriptMode=false] (TOFIX, #12)
    ...
    parseStatementHeader: function(keywordToken){
      var tok = this.tok;
      var lhp = tok.lastToken;
      lhp.isHeaderOpen = true;
      if (keywordToken) keywordToken.lhp = lhp;
      tok.mustBeNum(ORD_OPEN_PAREN, NEXTTOKENCANBEREGEX);
      this.parseExpressions();
      var rhp = tok.lastToken;
      rhp.isHeaderClose = true;
      lhp.rhp = rhp;
      if (keywordToken) {
        keywordToken.rhp = rhp;
        if (keywordToken.value === 'if') {
          rhp.isIfHeaderRhp = keywordToken;
        }
      }
    ...
    parseVar: function(){
      ...
      var tok = this.tok;
      var varToken = tok.lastToken;
      varToken.isVar = true;
      varToken.vars = [];
      tok.next(PUNC);
      do {
        varToken.vars.push(tok.lastToken.value);
    ...
    parseIf: function(inFunction, inLoop, inSwitch, labelSet){
      ...
      var tok = this.tok;
      var ifToken = tok.lastToken;
      ifToken.isIfToken = true;
      tok.next(PUNC);
      this.parseStatementHeader(ifToken);
      ifToken.ifStartToken = tok.lastToken;
      this.parseStatement(inFunction, inLoop, inSwitch, labelSet, REQUIRED, EMPTY_LABELSET);
      ifToken.ifEndToken = tok.lastToken;

      if (tok.getLastValue() === 'else') {
        ifToken.elseToken = tok.lastToken;
        tok.lastToken.isElse = true;
        tok.next(EXPR);
        ifToken.elseStartToken = tok.lastToken;
        this.parseStatement(inFunction, inLoop, inSwitch, labelSet, REQUIRED, EMPTY_LABELSET);
        ifToken.elseEndToken = tok.lastToken;
      }
    },
    parseDo: function(inFunction, inSwitch, labelSet, inLoop){
      ...
      doToken.isDo = true;
      tok.next(EXPR); // do
      this.parseStatement(inFunction, doToken, inSwitch, labelSet, REQUIRED, EMPTY_LABELSET);
      doToken.whileToken = tok.lastToken;
      doToken.whileToken.isDoWhileToken = true;
      tok.mustBeString('while', NEXTTOKENCANBEDIV);
      doToken.lhp = tok.lastToken;
      tok.mustBeNum(ORD_OPEN_PAREN, NEXTTOKENCANBEREGEX);
      this.parseExpressions();
      doToken.rhp = tok.lastToken;
      doToken.lhp.rhp = doToken.rhp;
      tok.mustBeNum(ORD_CLOSE_PAREN, NEXTTOKENCANBEREGEX);
      doToken.semiToken = tok.lastToken;

      // spec requires the semi but browsers made it optional
      if (this.options.requireDoWhileSemi || tok.firstTokenChar === ORD_SEMI) {
        this.parseSemi();
      }
      doToken.endToken = this.tok.lastToken;
    },
    parseWhile: function(inFunction, inSwitch, labelSet, inLoop){
      // while ( <exprs> ) <stmt>

      var whileToken = this.tok.lastToken;
      whileToken.isWhile = true;
      this.tok.next(PUNC);
      this.parseStatementHeader(whileToken);
      whileToken.whileStartToken = this.tok.lastToken;
      this.parseStatement(inFunction, whileToken, inSwitch, labelSet, REQUIRED, EMPTY_LABELSET);
      whileToken.whileEndToken = this.tok.lastToken;
      whileToken.endToken = this.tok.lastToken;
    },
    ...
    parseBreak: function(inLoop, inSwitch, labelSet){
      ...
      if (type !== IDENTIFIER || tok.lastNewline) {
        ...
        breakToken.inLoop = inLoop;
        breakToken.inSwitch = inSwitch;
      } else {
        throw 'not expecting break labels for this sub-project, assertion fail';
        var label = tok.getLastValue();
        if (!labelSet || labelSet.indexOf(' '+label+' ') < 0) {
          tok.throwSyntaxError('Label ['+label+'] not found in label set ['+labelSet+']');
        }
    ...
    parseReturn: function(inFunction){
      ...
      returnToken.argEndToken = tok.lastToken;
      this.parseSemi();
      returnToken.endToken = tok.lastToken;
    },
    parseThrow: function(){
      // throw <exprs> ;
    ...
    parseSwitch: function(inFunction, inLoop, labelSet){
      ...
      switchToken.isSwitch = true;
      tok.next(PUNC);
      this.parseStatementHeader();
      var lhc = tok.lastToken;
      ...
      while (this.parseStatement(inFunction, inLoop, switchToken, labelSet, OPTIONAL, EMPTY_LABELSET)) {
      ...
      lhc.rhc = tok.lastToken;
      tok.lastToken.isSwitchRhc = true;
    ...
    parseCase: function(inSwitch){
      ...
      caseToken.isCase = true;
      tok.next(EXPR);
      caseToken.caseStartToken = tok.lastToken;
      this.parseExpressions();
      caseToken.caseEndToken = tok.lastToken;
    ...
    parseFunction: function(forFunctionDeclaration){
      ...
      funcToken.isFunction = true;
      var type = tok.next(PUNC); // 'function'
      if (type === IDENTIFIER) { // name
        funcToken.name = this.tok.getLastValue();
      ...
      this.parseFunctionRemainder(ANYARGS, forFunctionDeclaration, funcToken);
    },
    /**
     ...
     */
    parseFunctionRemainder: function(paramCount, nextExpr, funcToken){
      var tok = this.tok;
      funcToken.lhp = tok.lastToken;
      funcToken.lhp.funcParams = true;
      tok.mustBeNum(ORD_OPEN_PAREN, NEXTTOKENCANBEDIV);
      this.parseParameters(paramCount);
      funcToken.rhp = tok.lastToken;
      tok.mustBeNum(ORD_CLOSE_PAREN, NEXTTOKENCANBEDIV);
      tok.lastToken.keepMe = true; // dont erase this curly
      this.parseCompleteBlock(nextExpr, INFUNCTION, NOTINLOOP, NOTINSWITCH, EMPTY_LABELSET, funcToken);
    },
    ...
    parseBlock: function(nextExpr, inFunction, inLoop, inSwitch, labelSet, funcToken){
      ...
      if (funcToken) {
        funcToken.rhc = this.tok.lastToken;
        this.tok.lastToken.keepMe = true;
      }
      this.tok.mustBeNum(ORD_CLOSE_CURLY, nextExpr);
    },
    parseCompleteBlock: function(nextExpr, inFunction, inLoop, inSwitch, labelSet, funcToken){
      if (funcToken) funcToken.lhc = this.tok.lastToken;
      this.tok.mustBeNum(ORD_OPEN_CURLY, NEXTTOKENCANBEREGEX);
      this.parseBlock(nextExpr, inFunction, inLoop, inSwitch, labelSet, funcToken);
    },
    parseSemi: function(){
      var tok = this.tok;
    ...
    parseAsi: function(){
      this.tok.throwSyntaxError('Assuming ASI-free code in this project!');
    ...
    parseExpressionOrLabel: function(labelName, inFunction, inLoop, inSwitch, labelSet, freshLabels){
      ...
      var tok = this.tok;
      var exprStart = tok.lastToken;
      exprStart.isExpressionStart = true;
      ...
      if (tok.firstTokenChar === ORD_COLON) {
        exprStart.isExpressionStart = false;
        ...
        tok.lastToken.isExpressionStop = true;
        exprStart.exprStop = tok.lastToken;
        if (tok.nextExprIfNum(ORD_COMMA)) {
          this.parseExpressions();
        }
        this.parseSemi();
      }
    ...
    parseExpressionOptional: function(){
      ...
      if (count !== beforeAssignments) {
        ...
        else if (beforeAssignments !== beforeNonAssignments) {
          assignable = ASSIGNABLEUNLESSGROUPED;
        }
...
//      7=80k
    ...
    parsePrimaryValue: function(optional){
      ...
      if (tok.firstTokenChar === ORD_OPEN_PAREN) {
        return this.parseGroup();
      }

      if (tok.firstTokenChar === ORD_OPEN_CURLY) {
        var lhc = tok.lastToken;
        lhc.keepMe = true;
        lhc.isObjlitStart = true;
        tok.next(EXPR);
        var rhc = this.parseObject();
        lhc.rhc = rhc;
        return NOTASSIGNABLE;
      }
    ...
    parseGroup: function(){
      ...
      var close = tok.lastToken;
      close.isGroupClose = true;
      open.rhp = close;
    ...
    parseObject: function(){
      ...
      var maybeComma;
      do {
        if (maybeComma) maybeComma.isObjectComma = true;
        var type = tok.lastType;
        if (type === IDENTIFIER || type === STRING || type === NUMBER) this.parsePair(); // 84% 9% 1% = 94%
        maybeComma = tok.lastToken;
      } while (tok.nextExprIfNum(ORD_COMMA)); // elision
